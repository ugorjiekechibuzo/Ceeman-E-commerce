    ...
    <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
      ...
      <div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>
        ...
        <div className='flex-1 flex flex-col-reverse gap-3 sm:flex-row'>
          <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal w-[18.7%] w-full'>
        ...
        <div className='flex-1'>
          ...
          <div className='flex items-center gap-1 mt-2'>
            ...
            <p className='text-gray-500 pl-2'>(3 Reviews)</p>
          </div>
          <p className='text-3xl font-semibold mt-4'>{currency}{productData.price}</p>
          <p className='text-gray-500 mt-4 '>{productData.description}</p>
          <div className='flex flex-col gap-4 my-8'>
            <p>Select Size</p>
            <div className='flex gap-2'>
              {productData.sizes.map((item, index) => (
                <button onClick={()=>setSize(item)}  key={index} className={`border border-gray-300 px-3 py-1 cursor-pointer ${item === size ? "border-orange-500" : ""}`}>{item}</button>
              ))}
            </div>
          </div>
