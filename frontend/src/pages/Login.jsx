...
const Login = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      {currentUser === "Login" ? "" : <input onChange={(e)=>setName(e.target.value)} value={name} type="text" className='w-full px-3 py-2 border border-gray-800' placeholder='Name' required/>}
      ...
      <div className='w-full flex justify-between text-sm mt-[-8px]'>
        ...
        {
          currentUser === "Login"
          ?<p onClick={() => setCurrentUser("Sign Up")} className='cursor-pointer'>Create Accout</p>
          : <p onClick={() => setCurrentUser("Login")} className='cursor-pointer'>Login Here</p>
        }
      </div>
